# Instalación de la Aplicación PROBOLIVIA (FRONTEND)
...
```sh
$ git clone git@gitlab.geo.gob.bo:agetic/mindesarrollo-probolivia-frontend.git
```
**Opción 2:** Si se quiere clonar el proyecto por HTTPS:
```sh
$ git clone https://usuario@gitlab.geo.gob.bo/agetic/mindesarrollo-probolivia-frontend.git
```
...
```sh
$ cd mindesarrollo-probolivia-frontend
```
...
Para asegurarse de que se instalen todas las librerías necesarias hay que configurar el ambiente en modo desarrollo. Después no olvidar cambiar a modo de producción.
```sh
...
```
Crear un archivo para hacer correr la aplicación de frontend, en este ejemplo, se definirá el archivo bajo el nombre de `proboliviaFrontendDEV`:
```sh
$ sudo touch proboliviaFrontendDEV.conf
```
**Para hacer correr la versión comprimida, colocar en el archivo el siguiente contenido:**
##### Sin variable de entorno
```sh
[program:proboliviaFrontendDEV]
directory=/home/usuario/rutaProyecto/PROBOLIVIA/mindesarrollo-probolivia-frontend
command=/home/usuario/.nvm/versions/node/v6.9.0/bin/npm run server
autostart=true
autorestart=true
stderr_logfile=/var/log/proboliviaFrontendDEV.err.log
stdout_logfile=/var/log/proboliviaFrontendDEV.out.log
user=usuario
```
