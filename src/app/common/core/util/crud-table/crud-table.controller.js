...
class CrudTableController {
    ...
    $onInit () {
        ...
        this.$scope.$on('refresh_crud_table', (ev, url) => { if (url==this.url) this.refresh(); });
    }
        ...
        if (this.data && this.data.list && this.Util.toType(this.data.list) == 'array') {
            ...
            this.tableParams = new this.NgTableParams({}, {
                getData: params => {
                    ...
                    if (this.queryExtra) {
                        angular.extend(query, this.queryExtra);    
                    }                    
    ...
    deleteRow(item) {
        if (typeof this.eventDelete == 'function') {
    ...
    getFieldClass(row, col) {
        if (this.clases && this.clases[col.field]) {
            return this.clases[col.field](row, col);
        } else {
            return ;
        }
    }
    ...
    buttonIf(event, item, callback) {
        if (typeof callback == 'function') {
            return  callback(event, item);
        } else {
            return true;
        }
    }
    ...
    actionChecks() {
        if (this.checks.cbButton) {
            this.checks.cbButton(this.tableParams.data);
        }
    }

    selectAllChecks () {
        this.tableParams.data.forEach( it => {  
            it.checked = this.modelSelectAllCheks;
            if (this.checks.if && !this.checks.if(it)) {
                delete it.checked;
            }
        })
        // this.$log.log(this.data)           
        // this.$log.log(this.data.list)
        // this.$log.log(this.data.list)
    }
    ...
    selectCheck (event, row) {
        ...
        this.modelSelectAllCheks = c==ct;

        if(this.checks.cbCheck) {
            this.checks.cbCheck(event, row);
        }
    }

    checkIf(row) {
        if (this.checks.if) {
            return this.checks.if(row);
        } else {
            return true;
        }
    }
