...
class DdjjController {
  constructor(NgTableParams, $log, Modal, Storage, DataService, $location, Util, $window, Message) {
    'ngInject';
  ...
  irDetalle(item) {
    this.$log.log('RESPUESTA CERTIFICACIONES = ', item);
    this.DataService.get(`certificaciones/${item.certificacion.id_certificacion}`)
    .then(response => {
      ...
      this.Storage.set('sw_detalle_certificacion', true);
    })
  }
