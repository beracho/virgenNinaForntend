...
class AuthService {
    ...
    constructor($http, $auth, $location, $rootScope, $uibModalStack, Storage, SidenavFactory, BreadcrumbFactory, Util, timeSessionExpired, Message, $log) {
        "ngInject";
    ...
    signin(data) {
        ...
        .then(response => {
            ...
            let rol = user.id_rol;
            let pathRedirect = response.data.menuEntrar;
            ...
            // Validaciones para el usuario EMPRESARIO
            if (user.estado == 'ACTIVO') {
                pathRedirect = response.data.menuEntrar ? response.data.menuEntrar : 'matricula';
                data.menu = true;
                data.sidenav = true;
            }
            ...
            if (error.data && error.data.mensaje) {
                ...
                this.Message.error('Los datos ingresados son incorrectos o su cuenta no está registrada.');
            }
        });
    }
    ...
    logout() {
        ...
        .then(() => {
            ...
            // Eliminando información de Probolivia
            this.Storage.remove('formulario');
            this.Storage.remove('matricula');
            this.Storage.remove('matriculas');
            this.Storage.remove('datos_matricula');
            this.Storage.remove('certificacion');
            this.Storage.remove('unidad_productiva');
            // Eliminando información de Promueve
            this.Storage.remove('matriculaPromueve');
            this.Storage.remove('matriculasPromueve');
